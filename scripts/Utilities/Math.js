...
//
// ------------------------------------------------------------------
MyGame.utilities.math = (function() {
    'use strict';
    let that = {};
    ...
    that.randomNumber = function(center, size, minOffset, maxOffset, absoluteMin, absoluteMax) {
        let max = center + (maxOffset * size);
        if (max > absoluteMax){
            max = absoluteMax;
        }
        let min = center - (maxOffset * size);
        if (min < absoluteMin) {
            min = absoluteMin;
        }
        let range = max - min;
        let randomPosition = center;
        while (randomPosition > center - (minOffset * size) && randomPosition < center + (minOffset * size)){
            randomPosition = Math.floor(min + (Math.random() * range));
        }
